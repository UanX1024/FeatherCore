                ...
                if let Ok(sub_entries) = fs::read_dir(&vendor_path) {
                    for board_entry in sub_entries.flatten() {
                        ...
                        if board_path.is_dir() {
                            let dts_path = board_path.join("config").join("devicetree.dts");
                            if dts_path.exists() {
                                if board_path.file_name()
...
pub fn generate_device_tree_info(dts_content: &str) -> Result<String, String> {
    ...
    // Try to find interrupt controllers
...
fn extract_string_property(line: &str) -> Option<String> {
    let start = line.find('"')?;
    let end = line[rfind_char(line, '"')? + 1..].find('"')?;
...
fn rfind_char(s: &str, c: char) -> Option<usize> {
    s.rfind(c)
}
