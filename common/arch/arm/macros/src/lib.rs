//! Internal implementation details of FeatherCore ARM runtime.
...
pub fn entry(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    // Extract static mut variables from the function body
    let (statics, stmts) = match extract_static_muts(f.block.stmts) {
        Err(e) => return e.to_compile_error().into(),
        Ok(x) => x,
    };

    f.sig.ident = Ident::new(&format!("__feather_core_arm_{}", f.sig.ident), Span::call_site());
    f.sig.inputs.extend(statics.iter().map(|statik| {
        let ident = &statik.ident;
        let ty = &statik.ty;
        let attrs = &statik.attrs;

        // Note that we use an explicit `'static` lifetime for the entry point arguments
        syn::parse::<FnArg>(
...
pub fn exception(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let exn = match &*ident_s {
        ...
        // NOTE that at this point we don't check if the exception is available on the target
    ...
    if f.sig.unsafety.is_none() {
        match exn {
            Exception::DefaultHandler | Exception::HardFault(_) | Exception::NonMaskableInt => {
                ...
                let name = format!("{}", exn);
                return parse::Error::new(ident.span(), format_args!("defining a {} is unsafe and requires an `unsafe fn`", name))
                    .to_compile_error()
                    .into();
            }
            Exception::Other => {}
    ...
    let assertion = match exn {
        Exception::Other => {
            quote! {
                const _: () = {
                    let _ = ::feather_core::arch::arm::Exception::#ident;
                };
            }
        }
        _ => quote!(),
        ...
        Exception::HardFault(args) => {
            ...
            if args.trampoline {
                ...
                quote!(
                    ...
                    unsafe extern "C" fn #tramp_ident(frame: &::feather_core::arch::arm::ExceptionFrame) {
                        #ident(frame)
                    }
        ...
        Exception::NonMaskableInt | Exception::Other => {
            ...
            f.block.stmts = iter::once(
                syn::parse2(quote! {{ 
                    ...
                    ::feather_core::arch::arm::exception::#ident;
                }})
                .unwrap(),
            )
            .chain(stmts)
...
pub fn pre_init(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    // Extract attributes, identifier, and block
    let attrs = f.attrs;
    let ident = f.sig.ident;
    let block = f.block;
...
fn check_attr_whitelist(attrs: &[Attribute], caller: WhiteListCaller) -> Result<(), TokenStream> {
    ...
    'o: for attr in attrs {
        ...
        let err_str = match caller {
            WhiteListCaller::Entry => "this attribute is not allowed on a feather-core-arm entry point",
            WhiteListCaller::Exception => {
                "this attribute is not allowed on an exception handler controlled by feather-core-arm"
            }
            WhiteListCaller::Interrupt => {
                "this attribute is not allowed on an interrupt handler controlled by feather-core-arm"
            }
            WhiteListCaller::PreInit => {
                "this attribute is not allowed on a pre-init controlled by feather-core-arm"
            }
        };
